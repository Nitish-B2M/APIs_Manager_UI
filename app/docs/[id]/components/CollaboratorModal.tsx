                ...
                <div className={`relative p-8 overflow-hidden ${theme === 'dark' ? 'bg-gradient-to-br from-indigo-900/40 via-[#1a1a2e] to-[#1a1a2e]' : 'bg-gradient-to-br from-indigo-50 via-white to-white'}`}>
                    ...
                    <div className="relative flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg ${theme === 'dark' ? 'bg-indigo-500/20 text-indigo-400 border border-indigo-500/30' : 'bg-indigo-600 text-white'}`}><Users size={24} /></div>
                            <div>
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-10 custom-scrollbar">
                            ...
                            <div className="mt-4 space-y-4 animate-in slide-in-from-top-2 duration-300">
                                <div className={`p-4 rounded-xl border ${theme === 'dark' ? 'bg-black/20 border-white/10' : 'bg-white border-gray-200'}`}>
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <input
                                            ...
                                            className={`flex-1 px-3 py-2 rounded-lg font-mono text-[11px] outline-none border focus:ring-2 focus:ring-indigo-500/20 ${theme === 'dark' ? 'bg-gray-900/50 border-white/5 text-indigo-400' : 'bg-gray-50 border-gray-200 text-indigo-600'}`}
                                            placeholder="my-awesome-api"
                                        />
                                        <button
                                            onClick={handleUpdateSlug}
                                            disabled={isUpdatingSlug || slugInput === slug}
                                            className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-[10px] font-black rounded-lg transition-all flex items-center gap-2"
                                        >
                                ...
                                {slug && (
                                    <div className={`p-4 rounded-xl border ${theme === 'dark' ? 'bg-black/20 border-white/10' : 'bg-white border-gray-200'}`}>
                                        ...
                                        <div className="flex items-center gap-2">
                                            <div className={`flex-1 px-3 py-2 rounded-lg font-mono text-[11px] truncate ${theme === 'dark' ? 'bg-gray-900/50 text-indigo-400' : 'bg-gray-50 text-indigo-600'}`}>{window.location.origin}/public/{slug}</div>
                                            <button onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/public/${slug}`); toast.success('Link copied!'); }} className="p-2 hover:bg-indigo-500/10 rounded-lg text-indigo-500 transition-all"><Copy size={16} /></button>
                    ...
                    {userRole === 'OWNER' ? (
                        <div className={`p-6 rounded-2xl border ${theme === 'dark' ? 'bg-indigo-500/5 border-indigo-500/20' : 'bg-gray-50 border-gray-200'}`}>
                            <label className={`text-[10px] font-black uppercase tracking-[0.2em] mb-4 block ${theme === 'dark' ? 'text-indigo-400' : 'text-indigo-600'}`}>Send Invitation</label>
                            <div className="flex flex-col sm:flex-row gap-4">
                                <div className="flex-1 relative group">
                                    <Mail className={`absolute left-3.5 top-1/2 -translate-y-1/2 transition-colors ${theme === 'dark' ? 'text-gray-600 group-focus-within:text-indigo-400' : 'text-gray-400 group-focus-within:text-indigo-500'}`} size={18} />
                                    <input type="email" value={inviteEmail} onChange={(e) => setInviteEmail(e.target.value)} placeholder="colleague@company.com" className={`w-full pl-11 pr-4 py-3 rounded-xl border text-sm font-medium transition-all focus:outline-none focus:ring-4 focus:ring-indigo-500/10 ${theme === 'dark' ? 'bg-[#0f0f1a] border-indigo-500/20 text-gray-200' : 'bg-white border-gray-300 text-gray-900'}`} />
                                </div>
                                <div className="relative min-w-[120px]">
                                    <select value={inviteRole} onChange={(e) => setInviteRole(e.target.value as UserRole)} className={`w-full px-4 py-3 rounded-xl border text-sm font-bold appearance-none cursor-pointer focus:ring-4 focus:ring-indigo-500/10 ${theme === 'dark' ? 'bg-[#0f0f1a] border-indigo-500/20 text-indigo-400' : 'bg-white border-gray-300 text-gray-700'}`}>
                                        <option value="VIEWER">Viewer</option><option value="EDITOR">Editor</option><option value="ADMIN">Admin</option>
                                    </select>
                                    <ChevronDown size={14} className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none opacity-50" />
                                </div>
                                <button onClick={() => inviteEmail && inviteMutation.mutate({ email: inviteEmail, role: inviteRole })} disabled={!inviteEmail || inviteMutation.isPending} className="px-8 py-3 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-sm font-black transition-all flex items-center justify-center gap-2">
                                    {inviteMutation.isPending ? <Loader2 size={18} className="animate-spin" /> : <UserPlus size={18} />} Invite
                                </button>
                            </div>
                        </div>
                    ) : (
                        <div className={`p-6 rounded-2xl border flex items-center justify-center gap-2 text-sm font-medium ${theme === 'dark' ? 'bg-indigo-500/5 border-indigo-500/20 text-indigo-400' : 'bg-gray-50 border-gray-200 text-indigo-600'}`}>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {collaborators.map((collab: any) => (
                                <div key={collab.id} className={`relative p-5 rounded-2xl border transition-all hover:shadow-xl overflow-hidden group ${theme === 'dark' ? 'bg-white/5 border-white/5 hover:border-indigo-500/30' : 'bg-white border-gray-100 hover:border-indigo-200'}`}>
                                    <div className={`absolute top-0 right-0 w-24 h-24 rounded-full blur-3xl -mr-10 -mt-10 opacity-0 group-hover:opacity-100 transition-opacity duration-500 ${theme === 'dark' ? 'bg-indigo-500/20' : 'bg-indigo-500/10'}`} />
                                    <div className="relative flex items-start justify-between">
                                        <div className="flex items-center gap-4">
                                            <div className="relative">
                                                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-lg shadow-sm ${theme === 'dark' ? 'bg-indigo-500/20 text-indigo-300' : 'bg-indigo-100 text-indigo-600'}`}>
                                            ...
                                            <div className="min-w-0">
                                                <div className="flex items-center gap-2 mb-0.5">
                                                    ...
                                                    <span className={`text-[9px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${collab.role === 'OWNER' ? 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400' :
                                                        collab.role === 'ADMIN' ? 'bg-purple-100 text-purple-700 dark:bg-purple-500/20 dark:text-purple-400' :
