    ...
    return (
        <div className="notes-tabs-container">
            {openNotes.map((note) => (
                ...
                >
                    {note.id === 'settings' ? (
                        <Settings size={14} className={`notes-tab-icon ${activeNoteId === note.id ? 'text-[#2ec4c7]' : ''}`} />
                    ) : (
                        <StickyNote size={14} className={`notes-tab-icon ${activeNoteId === note.id ? 'text-[#2ec4c7]' : ''}`} />
                    )}
